...
list_project(From, _To, #iq{xmlns = ?NS_AFT_PROJECT, sub_el = SubEl} = IQ)->
...
list_template(From, _To, #iq{xmlns = ?NS_AFT_PROJECT, sub_el = SubEl} = IQ)->
    {jid, U, S, _, _, _, _} = From,
    BaseJID = <<U/binary, "@", S/binary>>,
...
create(From, _To, #iq{xmlns = ?NS_AFT_PROJECT, sub_el = SubEl} = IQ) ->
    {jid, U, S, _, _, _, _} = From,
    BaseJID = <<U/binary, "@", S/binary>>,
    {struct,Data} = mochijson2:decode(xml:get_tag_cdata(SubEl)),
...
finish(From, _To, #iq{xmlns = ?NS_AFT_PROJECT, sub_el = SubEl} = IQ) ->
    {jid, U, S, _, _, _, _} = From,
    BaseJID = <<U/binary, "@", S/binary>>,
    {struct,Data} = mochijson2:decode(xml:get_tag_cdata(SubEl)),
...
list_children_jobs(From, _To, #iq{xmlns = ?NS_AFT_PROJECT, sub_el = SubEl} = IQ) ->
    {jid, U, S, _, _, _, _} = From,
    BaseJID = <<U/binary, "@", S/binary>>,
    {struct,Data} = mochijson2:decode(xml:get_tag_cdata(SubEl)),
    {_, ProID} = lists:keyfind(<<"project">>, 1, Data),
...
add_member(From, _To, #iq{xmlns = ?NS_AFT_PROJECT, sub_el = SubEl} = IQ) ->
    {jid, U, S, _, _, _, _} = From,
    BaseJID = <<U/binary, "@", S/binary>>,
    {struct,Data} = mochijson2:decode(xml:get_tag_cdata(SubEl)),
...
delete_member(From, _To, #iq{xmlns = ?NS_AFT_PROJECT, sub_el = SubEl} = IQ) ->
    {jid, U, S, _, _, _, _} = From,
    BaseJID = <<U/binary, "@", S/binary>>,
    {struct,Data} = mochijson2:decode(xml:get_tag_cdata(SubEl)),
    {_, ProID} = lists:keyfind(<<"project">>, 1, Data),
...
list_member(From, _To, #iq{xmlns = ?NS_AFT_PROJECT, sub_el = SubEl} = IQ) ->
    {jid, U, S, _, _, _, _} = From,
    BaseJID = <<U/binary, "@", S/binary>>,
    {struct, Data} = mochijson2:decode(xml:get_tag_cdata(SubEl)),
...
test(From, _To, #iq{xmlns = ?NS_AFT_PROJECT, sub_el = SubEl} = IQ) ->
    Data = xml:get_tag_cdata(SubEl),
    io:format("test Data=~p~n", [Data]),
    IQ#iq{type = result,
          sub_el = [SubEl#xmlel{attrs =
                [{<<"xmlns">>, ?NS_AFT_PROJECT},
                 {<<"type">>, <<"test">>}],
...
push_message(ProID, Server, ToList, Type, Contents) ->
    ...
    SubAttrs = [{<<"xmlns">>, ?NS_AFT_PROJECT}, {<<"type">>, Type},
        {<<"projectid">>, ProID}],

    Packet = {xmlel, <<"message">>, [], [{xmlel, <<"sys">>, SubAttrs, [{xmlcdata, Contents}]}]},
    lists:foreach(fun(ToIn) ->
        {To} = ToIn,
        ToAttr = {<<"to">>, To},

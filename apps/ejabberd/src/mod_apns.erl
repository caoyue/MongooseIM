...
connect(Connection) when is_record(Connection, apns_connection) ->
    mod_apns_sup:start_connection(Connection);
...
connect(Name, Connection) when is_record(Connection, apns_connection) ->
    mod_apns_sup:start_connection(Name, Connection);
...
-spec connect(
    atom(), fun((binary(), mod_apns:status()) -> stop | _), fun((string()) -> _)) ->
    {ok, pid()} | {error, {already_started, pid()}} | {error, Reason :: term()}.
...
estimate_available_bytes(#apns_msg{} = Msg) ->
    Payload = mod_apns_server:build_payload(Msg),
...
get_env(K, Def) ->
    case application:get_env(ejabberd, K) of
        {ok, V} -> V;
        _ -> Def
    end.

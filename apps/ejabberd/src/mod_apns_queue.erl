...
-spec in(QID :: pid(), Msg :: mod_apns:msg()) -> ok.
in(QID, Msg) ->
    gen_server:cast(QID, {in, Msg}).

-spec fail(QID :: pid(), ID :: binary()) -> [mod_apns:msg()].
fail(QID, ID) ->
...
-spec handle_call(X :: term(), reference(), state()) ->
    {reply, {Failed :: mod_apns:msg(), RestToRetry :: [mod_apns:msg()]}, state()}.
...
-spec recover_fail(ID :: binary(), Queue :: queue:queue()) -> {mod_apns:msg(), [mod_apns:msg()]}.
%@hidden
recover_fail(ID, Queue) ->
    Now = mod_apns:expiry(0),

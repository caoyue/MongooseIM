...
%% @hidden
-spec handle_cast(stop | mod_apns:msg(), state()) ->
...
handle_info({ssl, SslSocket, Data}
    ...
    case <<CurrentBuffer/binary, Data/binary>> of
        ...
        Token:Length/binary,
        Rest/binary>> ->
            try Feedback({mod_apns:timestamp(TimeT), bin_to_hexstr(Token)})
            catch
                _:Error ->
                    error_logger:error_msg(
